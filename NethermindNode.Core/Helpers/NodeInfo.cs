...
public static class NodeInfo
{
    ...
    private static readonly string[] IgnoredExceptionPatterns = new[]
    {
        ...
        "Receipt count mismatch with block transactions count", // SubprotocolException from Eth70ProtocolHandler.ValidateReceiptCount (Eth70ProtocolHandler.cs:624) when a PEER serves a receipts page whose count doesn't match the block's tx count during eth/70 Old-Receipts paging. This validation exists to REJECT bad peer data: Nethermind logs at WARN ("Failure when executing request"), discards the response, deprioritizes the peer and retries the batch elsewhere — local state is never touched. Same peer-quality class as "partial receipts response below minimum size" above. Confirmed benign on fully-synced, head-following nodes across 3 networks (1.39.3 validation run 30865692535: FuzzHT/FuzzHP/FuzzMT/FuzzMLo/FuzzML + SyncHNWS/SyncHNWSF). Not a node defect.
